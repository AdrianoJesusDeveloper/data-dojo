...
import { restoreHighlights, selectedRange } from "@/components/library/reader-selection";
...
function ReaderSession({ metadata }: { metadata: ReaderMetadata }) {
  const { book, toc, total } = metadata; const pdf = book.format === "pdf";
  const positions = pdf ? Array.from({ length: total }, (_, i) => i + 1) : toc.map(e => e.position);
  ...
  const schedule = useCallback((newPosition: number, newOffset: number) => {
    const index = positions.indexOf(newPosition); const entry = toc.find(e => e.position === newPosition);
  ...
  async function mark(kind: ReadingMark["kind"]) {
    ...
    if (note === null) return;
    try { await api.post(`${bookUrl(book.id)}marks/`, { kind, position, offset, location: toc.find(e => e.position === position)?.location ?? `page:${position}`, note, ...selection }); await client.invalidateQueries({ queryKey: ["reader-marks", book.id] }); setError(""); } catch (e) { setError(errorMessage(e)); }
  }
  ...
  return <div ref={root} className="flex h-dvh flex-col bg-background text-foreground">
    ...
    <div className="flex min-h-0 flex-1 flex-col md:flex-row">{sidebar && !focus && <div className="max-h-[40vh] overflow-auto md:max-h-none"><ReaderSidebar bookId={book.id} toc={toc} marks={marks.data?.results ?? []} onNavigate={navigate} onDelete={mark => void changeMark(mark)} onEdit={(mark, note) => void changeMark(mark, note)} />{marks.isError && <p role="alert">Não foi possível carregar marcações.</p>}{(marks.data?.next || marksPage > 1) && <div className="flex gap-2 p-3"><Button disabled={marksPage === 1} onClick={() => setMarksPage(marksPage - 1)}>Anteriores</Button><Button disabled={!marks.data?.next} onClick={() => setMarksPage(marksPage + 1)}>Mais marcações</Button></div>}</div>}
