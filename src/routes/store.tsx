...
function StorePage() {
  ...
  const categories = useMemo(() => Array.from(
    new Map(products.filter((product) => product.category).map((product) => [product.category!.slug, product.category!])).values(),
  ).sort((a, b) => a.name.localeCompare(b.name, "pt-BR")), [products]);

  const filteredProducts = useMemo(() => {
    const term = search.trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    return products.filter((product) => {
      const searchable = [product.name, product.short_description, product.category?.name, product.partner_name]
        .filter(Boolean).join(" ").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
      return (!term || searchable.includes(term))
        && (categoryFilter === "all" || product.category?.slug === categoryFilter)
        && (typeFilter === "all" || product.product_type === typeFilter)
        && (salesFilter === "all" || product.sales_model === salesFilter);
    });
  }, [products, search, categoryFilter, typeFilter, salesFilter]);
  ...
  useEffect(() => { (async () => {
    ...
    try {
      const { data } = await api.get<Product[] | { results: Product[] }>("/api/store/products/");
      const catalog = list(data);
      setProducts(catalog);
      setQuantities(Object.fromEntries(catalog.map((product) => [product.id, 1])));
      if (hasSession) {
