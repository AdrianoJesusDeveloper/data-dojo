...
import { ArrowLeft, ExternalLink, HelpCircle, LogIn, MessageCircle, Minus, Plus, Search, ShieldCheck, ShoppingBag, ShoppingCart, Star, Trash2, UserPlus, X } from "lucide-react";
...
function ProductVideo({ product }: { product: Product }) {
  ...
  try {
    ...
    if (url.hostname.includes("youtube.com")) embedUrl = `https://www.youtube-nocookie.com/embed/${url.searchParams.get("v") || ""}`;
    if (url.hostname === "youtu.be") embedUrl = `https://www.youtube-nocookie.com/embed/${url.pathname.slice(1)}`;
    if (url.hostname.includes("vimeo.com")) embedUrl = `https://player.vimeo.com/video/${url.pathname.split("/").filter(Boolean).pop() || ""}`;
  } catch { /* A API valida URLs; mantemos fallback seguro. */ }

  if (embedUrl) return <iframe src={embedUrl} title={`Vídeo de ${product.name}`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="mb-5 aspect-video w-full rounded-xl border border-border" />;
  if (/\.(mp4|webm|ogg)(\?.*)?$/i.test(product.video_url)) return <video src={product.video_url} controls preload="metadata" className="mb-5 aspect-video w-full rounded-xl border border-border bg-black" />;
  return <a href={product.video_url} target="_blank" rel="noreferrer" className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-kaizen hover:underline">Assistir ao vídeo <ExternalLink size={15} /></a>;
}
...
function StorePage() {
  ...
  async function approveSandboxPayment(order: Order) {
    if (!order.payment?.sandbox || order.status !== "pending") return;
    setBusy("checkout");
    try {
      const { data } = await api.post<Order>(order.payment.checkout_url);
      setOrders((current) => current.map((item) => item.id === data.id ? data : item));
      toast.success(`Pagamento sandbox do pedido #${order.id} aprovado. Nenhuma cobrança real foi feita.`);
    } catch (error: any) {
      toast.error(error?.response?.data?.detail || "Não foi possível simular a aprovação.");
    } finally { setBusy(null); }
  }
  ...
  return <div className="min-h-screen bg-background text-foreground"><Toaster position="top-right" theme="dark" richColors />
    <header className="sticky top-0 z-30 border-b border-border bg-background/95 shadow-[0_8px_30px_rgba(0,0,0,0.18)] backdrop-blur-xl">
      <div className="border-b border-kaizen/15 bg-kaizen/5">
        <div className="mx-auto flex max-w-7xl items-center justify-center gap-2 px-4 py-1.5 font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground sm:px-6">
          <ShieldCheck className="h-3.5 w-3.5 text-kaizen" aria-hidden="true" />
          <span>Loja oficial do ecossistema Data Driven Dojô</span>
        </div>
      </div>
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
        <Link to="/store" aria-label="Ir para o início da 3DStore" className="group flex min-w-0 items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-kaizen/30 bg-kaizen/10 transition group-hover:border-kaizen/60">
            <img src={logoDojo} alt="" className="h-9 w-9 object-contain" />
          </span>
          <span className="min-w-0">
            <span className="block truncate font-display text-lg font-extrabold leading-tight sm:text-xl">3DStore</span>
            <span className="hidden font-mono text-[9px] uppercase tracking-[.2em] text-kaizen sm:block">Arsenal para evolução</span>
          </span>
        </Link>

        <nav aria-label="Navegação da loja" className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          <a href="#catalogo" className="hidden rounded-lg px-3 py-2 text-sm text-muted-foreground transition hover:bg-secondary hover:text-foreground md:inline-flex">Catálogo</a>
          <a href="#carrinho" aria-label={`Abrir carrinho com ${cartItemCount} itens`} className="relative inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm transition hover:border-kaizen/50 hover:bg-kaizen/10 sm:px-4">
            <ShoppingCart className="h-4 w-4" aria-hidden="true" />
            <span className="hidden sm:inline">Carrinho</span>
            {cartItemCount > 0 && <span className="grid min-w-5 place-items-center rounded-full bg-kaizen px-1.5 py-0.5 font-mono text-[10px] font-bold text-kaizen-foreground">{cartItemCount}</span>}
          </a>
          {authenticated ? <button type="button" onClick={logout} className="rounded-lg border border-destructive/50 px-3 py-2 text-sm font-bold text-destructive transition hover:bg-destructive/10 sm:px-4">Sair</button> : <>
            <Link to="/login" search={{ redirect: "/store" }} className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm font-bold transition hover:bg-secondary sm:px-4"><LogIn className="h-4 w-4 sm:hidden" aria-hidden="true" /><span className="hidden sm:inline">Entrar</span></Link>
            <Link to="/register" className="hidden items-center gap-2 rounded-lg bg-kaizen px-4 py-2 text-sm font-bold text-kaizen-foreground transition hover:brightness-110 sm:inline-flex"><UserPlus className="h-4 w-4" aria-hidden="true" />Criar conta</Link>
          </>}
          <Link to="/" aria-label="Voltar ao Data Driven Dojô" title="Voltar ao Dojô" className="inline-flex items-center gap-2 rounded-lg px-2.5 py-2 text-sm text-muted-foreground transition hover:bg-secondary hover:text-foreground sm:px-3"><ArrowLeft className="h-4 w-4" aria-hidden="true" /><span className="hidden lg:inline">Dojô</span></Link>
        </nav>
      </div>
    </header>
