  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur supports-backdrop-filter:bg-background/70">
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-4">
        ...
        <nav className="ml-2 hidden md:flex items-center gap-1 overflow-x-auto">{navItem("/", "Início")}{navItem("/dashboard", "Dashboard")}{navItem("/workspace", "Workspace")}{navItem("/ai", "AI")}{navItem("/ai-sales", "Sales")}{navItem("/community", "Comunidade")}{navItem("/portfolio", "Portfólio")}{navItem("/conheca-sensey", "O Sensey")}{navItem("/store", "🛒 3DStore")}{navItem("/profile", "Meu Perfil")}<button onClick={handleLogout} className="px-3 py-1.5 rounded-md text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors ml-1 cursor-pointer whitespace-nowrap">Sair 🚪</button></nav>
      ...
      {mobileOpen && <div className="md:hidden border-t border-border bg-background px-4 py-3"><nav className="flex flex-col gap-1">{navItem("/", "Início")}{navItem("/dashboard", "Dashboard")}{navItem("/workspace", "Workspace")}{navItem("/ai", "AI")}{navItem("/ai-sales", "Sales")}{navItem("/community", "Comunidade")}{navItem("/portfolio", "Portfólio")}{navItem("/conheca-sensey", "O Sensey")}{navItem("/store", "🛒 3DStore")}{navItem("/profile", "Meu Perfil")}<button onClick={handleLogout} className="mt-2 px-3 py-2 text-left rounded-md text-sm font-medium text-destructive hover:bg-destructive/10">Sair 🚪</button></nav></div>}
    </header>
  );
}
