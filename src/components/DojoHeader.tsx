...
import { resolveAdministrativeAccess } from "@/lib/admin-access";
...
export function DojoHeader({ compact = false }: { compact?: boolean }) {
  ...
  const isAdmin = useAuthStore((auth) => auth.isStaff || auth.isSuperuser);
  const accessLoaded = useAuthStore((auth) => auth.accessLoaded);
  const token = useAuthStore((auth) => auth.token);
  const commandCenterUrl = import.meta.env.VITE_DOJO_DASHBOARD_URL || "https://dojo-dashboard-3ds.streamlit.app/";
  const showContentStudio = isAdmin && typeof window !== "undefined"
    && ["localhost", "127.0.0.1"].includes(window.location.hostname)
    && ["localhost", "127.0.0.1"].some((host) => API_ORIGIN.includes(host));
  useEffect(() => {
    if (token && !accessLoaded) void resolveAdministrativeAccess();
  }, [token, accessLoaded]);
  const handleLogout = () => { if (typeof window !== "undefined") { useAuthStore.getState().logout(); localStorage.removeItem("token"); localStorage.removeItem("profile_preview"); } navigate({ to: "/login" }); };
  ...
  if (rawPic && typeof rawPic === "string" && rawPic.trim() !== "") profilePic = rawPic.startsWith("http") || rawPic.startsWith("blob:") || rawPic.startsWith("data:") ? rawPic : `${API_ORIGIN}/media/${rawPic.replace(/^\/?(media\/)?/, "")}`;
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/85 backdrop-blur supports-backdrop-filter:bg-background/70">
      <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-4">
        <Link to="/" onClick={() => setMobileOpen(false)} className="flex items-center gap-2.5 group shrink-0"><div className="h-9 w-9 rounded-md bg-destructive flex items-center justify-center font-display font-black text-destructive-foreground text-lg shadow-[0_0_24px_rgba(230,57,70,0.45)]"><img src={logoDojo} width="36" height="36" alt="Logo" /></div><div className="leading-tight whitespace-nowrap"><div className="font-display font-bold tracking-tight">Data Driven Dojô</div><div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Determinação · Disciplina · Direção</div></div></Link>
<nav className="ml-2 hidden md:flex items-center gap-1 overflow-x-auto">{navItem("/", "Início")}{navItem("/dashboard", "Dashboard")}{navItem("/workspace", "Workspace")}{isAdmin && navItem("/professional-studio", "Project Studio")}{navItem("/ai", "AI")}{navItem("/ai-sales", "Sales")}{navItem("/community", "Comunidade")}{navItem("/portfolio", "Portfólio")}{navItem("/conheca-sensey", "O Sensey")}{navItem("/store", "🛒 3DStore")}{showContentStudio && navItem("/content-studio", "Content Studio")}{isAdmin && <a href={commandCenterUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-md text-sm font-medium text-muted-foreground hover:text-foreground whitespace-nowrap">Command Center ↗</a>}{navItem("/profile", "Meu Perfil")}<button onClick={handleLogout} className="px-3 py-1.5 rounded-md text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors ml-1 cursor-pointer whitespace-nowrap">Sair 🚪</button></nav>
        <div className="ml-auto flex items-center gap-3 min-w-0">
          ...
          <Link to="/store" aria-label="Abrir 3DStore" className="hidden sm:inline-flex h-10 w-10 items-center justify-center rounded-lg border border-kaizen/40 bg-kaizen/10 text-kaizen transition hover:bg-kaizen/20"><ShoppingBag className="h-5 w-5" /></Link>
      ...
      {mobileOpen && <div className="md:hidden border-t border-border bg-background px-4 py-3"><nav className="flex flex-col gap-1">{navItem("/", "Início")}{navItem("/dashboard", "Dashboard")}{navItem("/workspace", "Workspace")}{isAdmin && navItem("/professional-studio", "Project Studio")}{navItem("/ai", "AI")}{navItem("/ai-sales", "Sales")}{navItem("/community", "Comunidade")}{navItem("/portfolio", "Portfólio")}{navItem("/conheca-sensey", "O Sensey")}{navItem("/store", "🛒 3DStore")}{showContentStudio && navItem("/content-studio", "Content Studio")}{isAdmin && <a href={commandCenterUrl} target="_blank" rel="noreferrer" onClick={() => setMobileOpen(false)} className="px-3 py-1.5 rounded-md text-sm font-medium text-muted-foreground hover:text-foreground">Command Center ↗</a>}{navItem("/profile", "Meu Perfil")}<button onClick={handleLogout} className="mt-2 px-3 py-2 text-left rounded-md text-sm font-medium text-destructive hover:bg-destructive/10">Sair 🚪</button></nav></div>}
    </header>
  );
}
