...
export default function PdfReader({ url, page, zoom, width, fitPage, onRendered }: { url: string; page: number; zoom: number; width: number; fitPage: boolean; onRendered: () => void }) {
  ...
  return <Document file={file} loading={<p role="status">Carregando PDF…</p>} error={<p role="alert">Não foi possível renderizar este PDF.</p>} options={useMemo(() => ({ isEvalSupported: false }), [])}>
