...
def build_kpis():
    ...
    with engine.begin() as conn:
        ...
        return {"generated_at": datetime.now(timezone.utc).isoformat(), "users": users, "active_users": active_users, "courses": courses, "topics": topics, "comments": comments, "active_products": products, "orders": orders, "paid_orders": paid_orders, "revenue": float(revenue), "average_ticket": float(ticket)}
