...
def build_kpis():
    with engine().begin() as conn:
        ...
        paid_orders = scalar(conn, "SELECT COUNT(*) FROM store_order WHERE status IN ('paid','processing','fulfilled')")
        revenue = scalar(conn, "SELECT COALESCE(SUM(total), 0) FROM store_order WHERE status IN ('paid','processing','fulfilled')")
        revenue_today = scalar(conn, "SELECT COALESCE(SUM(total), 0) FROM store_order WHERE status IN ('paid','processing','fulfilled') AND created_at::date = CURRENT_DATE")
        ticket = Decimal(str(revenue)) / paid_orders if paid_orders else Decimal("0")
        conversion = (paid_orders / users * 100) if users else 0
        return {"generated_at": datetime.now(timezone.utc).isoformat(), "users": users, "active_users": active_users, "new_users_today": new_users_today, "courses": courses, "topics": topics, "comments": comments, "topics_today": topics_today, "comments_today": comments_today, "active_products": products, "orders": orders, "paid_orders": paid_orders, "revenue": float(revenue), "revenue_today": float(revenue_today), "average_ticket": float(ticket), "conversion_rate": float(conversion)}
...
def build_timeseries(days: int = 30):
    ...
    queries = {
        ...
        "revenue": "SELECT created_at::date AS day, COALESCE(SUM(total),0) AS value FROM store_order WHERE status IN ('paid','processing','fulfilled') AND created_at >= CURRENT_DATE - (:days * INTERVAL '1 day') GROUP BY 1 ORDER BY 1",
    }
